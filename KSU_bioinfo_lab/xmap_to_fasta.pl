...
#  perl xmap_to_fasta.pl sample.xmap sample_scaffold.fasta
...
my $scaffold_id = "Scaffold_$n"; ### initialize first superscaffold
...
for my $row (@xmap_table)
{
	unless (exists $finished{$row->[1]})
    {
        ...
        if ($new_mol != $old_mol) ## if we are not on the same molecule
        {
            ...
            $scaffold_id = "Scaffold_$n"; ## initialize new superscaffold
            $new_seq = '';
            ++$n;
        }
