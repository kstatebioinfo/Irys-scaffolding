...
####################################################################
my $third_mqr = `~/tools/RefAligner -i ${bnx_dir}/all_flowcells_adj_merged.bnx -o ${bnx_dir}/all_flowcells_adj_merged -T ${T} -ref ${ref} -nosplit 2 -M 5 -biaswt 0 -Mfast 0 -FP 1.5 -FN 0.15 -sf 0.2 -sd 0.2 -A 5 -S -1000 -res 3.5 -resSD 0.7 -outlier 1e-4 -endoutlier 1e-4 -minlen 150 -minsites 5`;
print "$third_mqr";
    ...
    while (<ERR>)
    {
        if (eof)
        {
            ...
            if ($values[7] != 0)
            {
                ...
                print FLOWCELL_BNX_SUMMARY "$file,$values[1],$values[2],$values[11],$values[7],$values[9],$map_ratio\n";
            }
            else
            {
